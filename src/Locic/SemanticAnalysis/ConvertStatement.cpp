...
SEM::Statement * ConvertStatement(LocalContext& context, AST::Statement * statement){
	switch(statement->type){
		case AST::Statement::VALUE:
		{
			...
			if(value != NULL){
				return SEM::Statement::Value(value);
			}
			return NULL;
		}
		case AST::Statement::SCOPE:
		{
			...
			return SEM::Statement::Scope(scope);
		}
		case AST::Statement::IF:
		{
			SEM::Value * cond = ConvertValue(context, statement->ifStmt.cond);
			...
			if(boolValue == NULL){
				printf("Semantic Analysis Error: Cannot cast or copy condition type (%s) to bool type in IF statement.\n",
		...
		{
			SEM::Value * cond = ConvertValue(context, statement->whileStmt.cond);
			...
			if(boolValue == NULL){
				printf("Semantic Analysis Error: Cannot cast or copy condition type (%s) to bool type in WHILE statement.\n",
					cond->type->toString().c_str());
				return NULL;
			}
		...
		case AST::Statement::VARDECL:
		{
			...
			return SEM::Statement::Assignment(SEM::Value::Var(semVar), castValue);
		}
