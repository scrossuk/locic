...
namespace Locic {
	...
	namespace CodeGen {
		...
		class Function {
			public:
				inline Function(Module& module, llvm::FunctionType* type,
					llvm::GlobalValue::LinkageTypes linkage, const std::string& name, const ArgInfo& argInfo)
					: module_(module),
					function_(llvm::Function::Create(type, linkage, name, module.getLLVMModulePtr())),
					builder_(module.getLLVMContext()), argInfo_(argInfo) {
						assert(argInfo_.numArguments() == type->getNumParams());
					}
