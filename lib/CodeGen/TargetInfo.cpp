...
namespace locic {
	...
	namespace CodeGen {
		...
		TargetInfo::TargetInfo(const std::string& triple)
			...
			primitiveSizes_.insert("ptrdiff_t", clangTargetInfo->getTypeWidth(clangTargetInfo->getSizeType()));
			...
			primitiveSizes_.insert("compare_result_t", clangTargetInfo->getCharWidth());
			
			primitiveSizes_.insert("byte_t", 8);
			primitiveSizes_.insert("ubyte_t", 8);
